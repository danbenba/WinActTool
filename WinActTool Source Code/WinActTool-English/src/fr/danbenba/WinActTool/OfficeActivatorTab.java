...
public class OfficeActivatorTab {
	...
	private static ImageView loadIcon(String path) {
	    ...
	    try {
	        ...
	        if (iconStream != null) {
	            ...
	            System.out.println(red + "[WinActTool] ERROR: Icon path not found. Path: " + path + reset);
	        }
	        } catch (Exception e) {
	        e.printStackTrace();
	                 // ANSI code for red text
	                     String red = "\033[31m"; // 31 is the code for red
	                     String reset = "\033[0m"; // Reset color
	                    
	        System.out.println(red + "[WinActTool] ERROR: Loading icon: " + e.getMessage() + reset);
	    }
	    return iconView;
	}
	...
	public static VBox createCrackerTab(Stage primaryStage) {
        ...
        Label buttonDescription = new Label("The official activation tool for Microsoft Office 2021 using KMS method.");
	...
	private static void executeAndDisplayCracker(Label ofatitleLabel, VBox hidbox, TextArea terminalPreview, Node textFlow, Button crackerButton, VBox mainLayout) {
		...
		if (!isRunAsAdmin()) {
			...
			Platform.runLater(() -> {
			    ...
			    alert.setContentText("The script must be run as administrator.");
        ...
        new Thread(() -> {
        	try {
                ...
                if (inputStream == null) {
                    ...
                    String red = "\033[31m"; // 31 est le code pour le rouge
                    String reset = "\033[0m"; // Réinitialiser la couleur
                    Platform.runLater(() -> terminalPreview.appendText(red + "[WinActTool] ERROR: Activator file not found in resources.\n" + reset));
                    return;
                }
                Platform.runLater(() -> {
            		appendCenteredText(terminalPreview, "---------LOGS---------");
            		terminalPreview.appendText("[WinActTool] LOGS: Verifying activator OK\n");
                    terminalPreview.appendText(" \n");
            });
            ...
            appendCenteredText(terminalPreview, "---------LOGS---------\n");
                terminalPreview.appendText("[WinActTool] LOGS: Verifying activator OK\n");
                    terminalPreview.appendText(" \n");
                    terminalPreview.appendText("[WinActTool] LOGS: Verifying the automatic installation\n");
                    terminalPreview.clear();
            		});
             ...
             // Étape 2 : Vérifier si Office 2021 est installé
                ...
                if (!isOffice2021Installed) {
                    ...
                    String red = "\033[31m"; // 31 est le code pour le rouge
                    String reset = "\033[0m"; // Réinitialiser la couleur
                    Platform.runLater(() -> terminalPreview.appendText(red + "[WinActTool] ERROR: Office 2021 is not installed or not found.\n" + reset));
                    return;
                }
                Platform.runLater(() -> {
            appendCenteredText(terminalPreview, "---------LOGS---------");
            ...
            terminalPreview.appendText("\n");
                terminalPreview.appendText("[WinActTool] LOGS: Verifying activator OK\n");
                terminalPreview.appendText("[WinActTool] LOGS: Verifying the automatic installation. OK\n");
                terminalPreview.appendText(" \n");
            ...
            terminalPreview.appendText("\n");
                ...
                Platform.runLater(() -> {
                	...
                	// Display a temporary message or perform another action
                    Text successTextFlow = new Text("Preparing the last step...\n");
                ...
                try {
                    ...
                    if (additionalTaskStream == null) {
                    	Platform.runLater(() -> terminalPreview.appendText("The additionalTask.bat file was not found in resources.\n"));
                        return;
                    }
