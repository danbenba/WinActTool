...
public class WindowsActivatorTab {
	...
	private static ImageView loadIcon(String path) {
	    ...
	    try {
	        ...
	        if (iconStream != null) {
	            ...
	            System.out.println("Error: Icon path not found. Path: " + path);
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	...
	private static void executeAndDisplayCrackerBasedOnSelection(Label buttonDescription, VBox terminalBox, TextArea terminalPreview, Node textFlow, Button crackerButton, ComboBox<String> windowsVersionComboBox) {
	    ...
	    switch (selectedVersion) {
	        case "KMS Activation":
	            batchFileName = "/files/KMS-Activator.exe";
	            break;
	        case "HWID Activation":
	            batchFileName = "/files/HWID-Activation.bat";
	            break;
	        default:
	            batchFileName = "/files/KMS-Activator.exe"; // Fichier par défaut
	            break;
	    }

	    // Affiche le nom du fichier batch sélectionné dans le terminalPreview
	    appendCenteredText(terminalPreview, "Activating using: " + batchFileName);

	    // Exécution du fichier batch sélectionné
	    executeBatchFile(batchFileName, terminalPreview);
	}

	private static void executeBatchFile(String batchFileName, TextArea terminalPreview) {
	    try {
	        // Chemin complet du fichier batch dans les ressources
	        URL batchFileURL = WindowsActivatorTab.class.getResource(batchFileName);
	        if (batchFileURL == null) {
	            appendCenteredText(terminalPreview, "Batch file not found: " + batchFileName);
	            return;
	        }

	        File batchFile = new File(batchFileURL.toURI());

	        ProcessBuilder processBuilder = new ProcessBuilder("cmd.exe", "/c", batchFile.getPath());
	        ...
	        while ((line = reader.readLine()) != null) {
	            final String finalLine = line; // Créez une nouvelle variable finale
	            Platform.runLater(() -> terminalPreview.appendText(finalLine + "\n"));
	        }

	        int exitCode = process.waitFor();
	        Platform.runLater(() -> terminalPreview.appendText("Process exited with code " + exitCode + "\n"));
	    } catch (Exception e) {
	        Platform.runLater(() -> terminalPreview.appendText("Error: " + e.getMessage() + "\n"));
	    }
	}
	...
	public static VBox createCrackerTab() {
        ...
        Label buttonDescription = new Label("L'outil d'activation officiel pour Windows 8-11.");
     ...
     // Création de la ComboBox
        ...
        windowsVersionComboBox.getItems().addAll("KMS Activation", "HWID Activation");
